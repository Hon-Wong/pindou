...
tests/e2e.html   浏览器端到端测试：在同源 iframe 里驱动真实的 index.html（112 项）
...
量化质量不低于「色卡理论下限 + 1.5 ΔE」、边缘保留采样确实减少中间色、撤销重做（含禁用颜色后撤销）、
...
- 试过写一个「更能保留细节的量化算法」，实测三种思路都更差，已放弃（`js/quantize.js` 里留了记录）。
